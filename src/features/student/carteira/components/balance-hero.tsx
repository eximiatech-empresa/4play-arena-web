...
export function BalanceHero({ balance, totalPlays }: BalanceHeroProps) {
  const level = getBalanceLevel(balance, totalPlays)
  const progressPct = totalPlays > 0 ? Math.round((balance / totalPlays) * 100) : 0

  return (
    <div className="relative overflow-hidden rounded-2xl bg-chart-4 p-6 sm:p-8">
      ...
      <div className="relative z-10">
        ...
        <div className="mt-2 flex items-end gap-2">
          ...
          <span className="text-xl font-medium text-white/60 mb-1.5">Plays</span>
        </div>
        ...
        <div className="mt-5">
          <div className="flex items-center justify-between mb-1.5 text-[10px] text-white/40 font-medium">
            <span>0 Plays</span>
            <span>{totalPlays.toFixed(1)} Plays contratados</span>
          </div>
